 ...
 */
package com.afis.jx.ckfinder.connector;
...
import com.afis.jx.ckfinder.connector.configuration.Configuration;
import com.afis.jx.ckfinder.connector.configuration.ConfigurationFactory;
...
import com.afis.jx.ckfinder.connector.data.BeforeExecuteCommandEventArgs;
import com.afis.jx.ckfinder.connector.errors.ConnectorException;
import com.afis.jx.ckfinder.connector.handlers.command.Command;
import com.afis.jx.ckfinder.connector.handlers.command.CopyFilesCommand;
import com.afis.jx.ckfinder.connector.handlers.command.CreateFolderCommand;
import com.afis.jx.ckfinder.connector.handlers.command.DeleteFilesCommand;
import com.afis.jx.ckfinder.connector.handlers.command.DeleteFolderCommand;
import com.afis.jx.ckfinder.connector.handlers.command.DownloadFileCommand;
import com.afis.jx.ckfinder.connector.handlers.command.ErrorCommand;
import com.afis.jx.ckfinder.connector.handlers.command.FileUploadCommand;
import com.afis.jx.ckfinder.connector.handlers.command.GetFilesCommand;
import com.afis.jx.ckfinder.connector.handlers.command.GetFoldersCommand;
import com.afis.jx.ckfinder.connector.handlers.command.IPostCommand;
import com.afis.jx.ckfinder.connector.handlers.command.InitCommand;
import com.afis.jx.ckfinder.connector.handlers.command.MoveFilesCommand;
import com.afis.jx.ckfinder.connector.handlers.command.QuickUploadCommand;
import com.afis.jx.ckfinder.connector.handlers.command.RenameFileCommand;
import com.afis.jx.ckfinder.connector.handlers.command.RenameFolderCommand;
import com.afis.jx.ckfinder.connector.handlers.command.ThumbnailCommand;
import com.afis.jx.ckfinder.connector.handlers.command.XMLCommand;
import com.afis.jx.ckfinder.connector.handlers.command.XMLErrorCommand;
import com.afis.jx.ckfinder.connector.utils.AccessControlUtil;
...
public class ConnectorServlet extends HttpServlet {
	 ...
	 *            response
	 * @param post
	 *            if it's post command.
	 * @throws ServletException
	 *             when error occurs.
	 */
	private void getResponse(final HttpServletRequest request, final HttpServletResponse response, final boolean post)
			throws ServletException {
		...
		try {
			...
			if (Boolean.valueOf(getServletConfig().getInitParameter("debug"))) {
				Logger.getLogger(ConnectorServlet.class.getName()).log(Level.SEVERE,
						"Configuration wasn't initialized correctly. Check server logs.", e);
			}
		...
		try {
			...
			if (CommandHandlerEnum.contains(command.toUpperCase())) {
				...
				// checks if command should go via POST request or it's a post
				// request
				// and it's not upload command
				if ((cmd.getCommand() instanceof IPostCommand || post) && !CommandHandlerEnum.FILEUPLOAD.equals(cmd)
			...
			if (Boolean.valueOf(getServletConfig().getInitParameter("debug"))) {
				Logger.getLogger(ConnectorServlet.class.getName()).log(Level.SEVERE, "Couldn't execute native command.",
						e);
				response.reset();
				throw new ServletException(e);
			} else {
				handleError(new ConnectorException(Constants.Errors.CKFINDER_CONNECTOR_ERROR_INVALID_COMMAND, false),
						configuration, request, response, command);
			}
		} catch (ConnectorException e) {
			if (Boolean.valueOf(getServletConfig().getInitParameter("debug"))) {
				Logger.getLogger(ConnectorServlet.class.getName()).log(Level.SEVERE, e.getErrorMessage(),
	 ...
	 *
	 * @param command
	 *            string representing command name
	 * @param request
	 *            current request object
	 * @param respose
	 *            current response object
	 * @param configuration
	 *            CKFinder connector configuration
	 * @param isNativeCommand
	 *            flag indicating whether command is available in enumeration
	 *            object
	 *
	 * @throws ConnectorException
	 *             when command isn't native
	 * @throws IllegalArgumentException
	 *             when provided command is not found in enumeration object
	 */
	private void executeNativeCommand(String command, final HttpServletRequest request,
			final HttpServletResponse response, IConfiguration configuration, boolean isNativeCommand)
	 ...
	 *            request
	 * @throws ConnectorException
	 *             when param isn't set or has wrong value.
	 */
	private void checkPostRequest(final HttpServletRequest request) throws ConnectorException {
	 ...
	 */
	private void handleError(final ConnectorException e, final IConfiguration configuration,
			final HttpServletRequest request, final HttpServletResponse response, final String currentCommand)
			throws ServletException {
		try {
	...
	public void init() throws ServletException {
		...
		try {
			...
			if (className != null) {
				...
				if (clazz.getConstructor(ServletConfig.class) != null) {
					configuration = (IConfiguration) clazz.getConstructor(ServletConfig.class)
		...
		try {
			...
			if (Boolean.valueOf(getServletConfig().getInitParameter("debug"))) {
				Logger.getLogger(ConnectorServlet.class.getName()).log(Level.SEVERE,
						"Couldn't initialize configuration object.", e);
			}
	...
	private enum CommandHandlerEnum {
		...
		/**
		 * Fills in {@code Set} holding enumeration values for this
		 * {@code Enum}.
		 */
		 ...
		 *
		 * @param enumValue
		 *            string representing command name to check
		 *
		...
		private void execute(final HttpServletRequest request, final HttpServletResponse response,
				final IConfiguration configuration, final ServletContext sc, final Object... params)
				throws ConnectorException {
			Command com = null;
			try {
